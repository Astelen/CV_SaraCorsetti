...
def reverse_string(stringa):
    stringa_inverso = ""
    for char in stringa:
        stringa_inverso = char + stringa_inverso
    return stringa_inverso

stringa_da_ribaltare = "Prova 1234 vediamoooo"
print(reverse_string(stringa_da_ribaltare))



#Trasformazione numero in binario
a = 10
b = 3
c = 27
...
#Binario con ciclo while
def converti_binario(numero):
    binario = ""
    while numero > 0:
        if numero%2 == 0:
            binario = "0" + binario
...
numero = int(input("Inserisci un numero: "))
print(converti_binario(numero))


#Dividi cifre in lista
def dividi_cifre(numero):
    lista_cifre = []
    for num in str(numero):
        lista_cifre.append(int(num))
    return lista_cifre

numero_per_binario = dividi_cifre(456)
print(numero_per_binario)


#Lamba: quanti argomenti vuoi, una cosa esecuzione
x = lambda a, b: a*b + 10
print(x(5, 2))
print(x(10, 4))
...
#Convertire lista in dizionario
lista_diz = [1, "a", 2, "b", 3, "c"]
diz_lista = {}
for ele in range(0, len(lista_diz), 2):
    diz_lista[lista_diz[ele]] = lista_diz[ele + 1]
...
palindromo("ciaone a tutti")
palindromo("O mordo tua nuora o aro un autodromo")